...
def design_single(
    ...
    for epo in range(1, max_epo+1):
        ...
        if len(old_best_seq_list) == 0:
            ...
            init_seq_list = random.choices(old_best_seq_list, weights=old_best_score_list, k=n_gen)
                ...
                for t in range(1, max_step + 1):
                    ...
                    if final_step:
                        ...
                        if len(new_best_seq_list) > max_seq_log:
                            ...
                            select_index = random.choices(all_index_list, weights=new_best_score_list, k=max_seq_log)
...
if __name__ == "__main__":
    ...
    log_folder = "./Design_Log/" + loc_time + "_v{}/".format(fold_version)
    os.mkdir(log_folder)

    design_single(
        dotB_id=dotB_id,
        dotB_folder="./data/eterna_v2",
        n_gen=n_gen,
        restart_epo=restart_epo,
        max_epo=max_epo,
        max_step=max_step,
        agent_dir="/amax/data/liyichong/RNA_Split_2_data/2024_01_08_17_07_38_V2/models/120_1/",
        fold_version=2,
        gpu_order=3,
