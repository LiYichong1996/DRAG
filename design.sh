...
do
    ...
    do
        ...
        then
            echo $epi_f
            python ${LOCAL_DIR}/design_epi.py --dotB_dir ${DATA_DIR} --best_dir ${best_dir} --id_batch_str ${id_batch_str} --done_dir ${done_dir} --agent_dir ${LOAD_MODEL_DIR} --episode ${epi} --step ${STEP} --gpu_order ${GPU} --n_traj ${N_TRAJ} --n_step ${N_STEP} --final_traj ${F_TRAJ} --final_step ${F_STEP} --use_task_pool --slack_threshold ${SLACK_THRESHOLD} --fold_version ${RNAfold_Version} --use_freeze --init_seed ${init_seed} #--use_mp 
        else
            echo "Renew solutions."
            python ${LOCAL_DIR}/design_epi.py --dotB_dir ${DATA_DIR} --best_dir ${best_dir} --id_batch_str ${id_batch_str} --done_dir ${done_dir} --agent_dir ${LOAD_MODEL_DIR} --episode ${epi} --step ${STEP} --load_best --gpu_order ${GPU} --n_traj ${N_TRAJ} --n_step ${N_STEP} --final_traj ${F_TRAJ} --final_step ${F_STEP} --use_task_pool --slack_threshold ${SLACK_THRESHOLD} --fold_version ${RNAfold_Version} --use_freeze #--use_mp 
        fi
    done
done