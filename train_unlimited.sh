...
do
    ...
    #     python ./utils/get_ids.py --done_dir ${done_dir_f} --n_rna ${n_rna} --batch_size ${PLAY_BATCH_SIZE} --save_dir ${id_batch_dir} --forbid_id 22 --maintain_solved
    # else
    echo "Remove solved puzzles."
    python ./utils/get_ids.py --done_dir ${done_dir_f} --n_rna ${n_rna} --batch_size ${PLAY_BATCH_SIZE} --save_dir ${id_batch_dir} --forbid_id 22
    # fi
    ...
    do
        ...
        if [ "$epi" -eq 1 ] # && [ "$epo" -eq 0 ]
        then
            load_model_dir="/amax/data/liyichong/RNA_Split_2_data/2024_01_08_17_07_38_V2/models/120_1/"
        fi
